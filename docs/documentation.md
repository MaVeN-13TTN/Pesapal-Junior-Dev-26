...
We chose **Python** for this challenge for several strategic reasons:

1.  **Rapid Prototyping**: Python's dynamic nature allowed us to build a working Parser, Storage Engine, and Web API in under 4 hours.
...
3.  **Data Structures**: Python's Dictionaries (`dict`) are highly optimized hash maps. Using them as the backbone for our In-Memory storage (`Table` rows and `Index`) ensures O(1) lookups for Primary Keys by default.
4.  **Standard Library**: Modules like `cmd` (for the REPL) and `json` (for persistence) allowed us to implement complex features with zero external dependencies for the core engine.
...
### Query Processing
- **`src/parser/parser.py`**: A custom Recursive-Descent/Regex parser. It takes raw SQL strings and converts them into structured `Command` objects (e.g., `SelectCommand`, `InsertCommand`). This decoupling means we can easily swap the syntax without breaking the execution logic.
